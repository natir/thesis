...
\jsv{j'appuie la remarque de Rayan : il faut citer les overlappers qui utilisent cette stratégie, même si tu dis qu'ils seront detallés plus tard (minimap2, mhap)}

\jsv{1 citer des mapper qui utilisent cette stratégie mais pas pour de la 3rd generation}
...
\jsv{2 citer des mapper qui utilisent cette stratégie pour de la 3rd generation}
We will give some details on \mhap and \minimap in section \ref{chapter:sota}.
For third generation reads, the length of the reads and the large number of errors make the choice of algorithm parameters even more complicated, particularly concerning the seed \jsv{tu veux duire le type de graine ? si oui il faut expliquer avant qu'il y en a plusieurs types} and length of seeds.
\jsv{il faut conclure avec l'intérêt de cette stratégie}

To found overlap, similar sequence in a read dataset, FM-index was used too, for example \toolsname{SGA} \cite{SGA} to search exact overlap between low error read, by search a sub-sequence at end of read in a FM-index. 
\jsv{franchement je ne sais pas quoi faire de cette phrase, qu'apporte-t-elle d'autre que OK, tu connais une datastructure de plus, les FM-index}
