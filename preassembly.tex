...
With NGS technology development more and more data had to be processed and the seed-and-extend strategy was replaced by a seed-only strategy. Indeed, the extension step is still very time-consuming. With the seed-and-extend strategy, the quality of an overlap is evaluated in relation to its length and number of errors. With the seed-only strategy, we can only rely on the number of seeds and their positions to evaluate overlap. For third generation reads, the length of the reads and the large number of errors made the choice of algorithm parameters even more complicated, particularly as regards on how we choose the seed and length of seeds.
